from flask import Flask, render_template, request, redirect, url_for,jsonify, flash,  send_file, session as flask_session
from flask_sqlalchemy import SQLAlchemy
from werkzeug.security import generate_password_hash, check_password_hash
from configparser import ConfigParser
from flask_jwt_extended import jwt_required,create_access_token,JWTManager ,  get_jwt_identity, get_jwt# type: ignore
from configparser import ConfigParser
...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:lavakishor14@localhost/RMT_main_db'
...
def register():
    if request.method == 'POST':
        ...
        # Create new Employee instance and add to database
        new_employee = Employee(name=name, email=email, position=position, password=password_hash)
        db.session.add(new_employee)
        db.session.commit()

        # Assuming send_registration_email() function is defined elsewhere
        send_registration_email(email, name)

        # Redirect to success page with relevant data
        return redirect(url_for('registersuccess', name=name, email=email, password=password, position=position))

    # Render the registration form if method is GET (optional, if needed)
    return render_template('register.html')

@app.route('/registersuccess')
def registersuccess():
    # Example of accessing passed parameters in the success page
    name = request.args.get('name')
    email = request.args.get('email')
    position = request.args.get('position')
    password = request.args.get('password')

    return render_template('registersuccess.html', name=name, email=email, position=position, password=password)
    ...
    if request.method == 'POST':
        ...
        if user and user.check_password(password):
            if user.position == 'Account Manager':
                ...
                # Store access token in session or however you handle authentication
                ...
                return render_template('login.html', loginerror="No authentication for login. Please contact Admin.")

        else:
            return render_template('login.html', loginerror="Invalid email or password.")
...
# @jwt_required()
# def logout():
#     jti = get_jwt()['jti']  # JWT ID, a unique identifier for the JWT
#     blocklist.add(jti)
...
def logout():
    if request.method == 'POST':
        ...
        session.clear()  # Clear Flask session data if used
...
def add_secondary_info():
    ...
    tm_spocs = Employee.query.filter_by(position='TM SPOC').all()
    sl_pocs = Employee.query.filter_by(position='SL POC').all()
    return render_template('dashboard1.html', tm_spocs=tm_spocs, sl_pocs=sl_pocs)
...
def view_resource(emp_id):
    ...
    print(emp_id)
    # secondary_info = ResourceSecondaryInfo.query.get(resource.emp_id)
    ...
    # return render_template('resource_details.html', resource=resource,  get_employee_name=get_employee_name, data = secondary_info)
...
@app.route('/update_secondary_info/<int:id>', methods=['POST'])
...
@app.route('/update_resource_manager/<int:id>', methods=['GET', 'POST'])
# @jwt_required
def update_resource_manager(id):
    resource = Resource.query.get_or_404(id)
    if request.method == 'POST':
        resource.employee_name = request.form['employee_name']
        resource.email_id = request.form['email_id']
        ...
        if  secondary_info == None:
            ...
            sl_pocs = Employee.query.filter_by(position='SL POC').all()
            return render_template('add_secondary_info.html', tm_spocs=tm_spocs, sl_pocs=sl_pocs, resource_id = emp_id)
        return render_template('update_secondary_info_by_manager.html', secondary_info = secondary_info, tm_spocs=tm_spocs, sl_pocs=sl_pocs)
        # return redirect(url_for('view_resources'))
    managers = Employee.query.filter_by(position='Account Manager').all()
    return render_template('update_resource_by_manager.html', resource=resource, managers=managers)

@app.route('/update_secondary_by_manager/<int:id>', methods=['POST'])
def update_secondary_by_manager(id):
    secondary_info = ResourceSecondaryInfo.query.get_or_404(id)
    if request.method == 'POST':
        secondary_info.experience_level = request.form['experience_level']
        secondary_info.doj = datetime.strptime(request.form['doj'], '%Y-%m-%d')
        ...
        db.session.commit()
        flash('Secondary info updated successfully', 'success')
        # resource = session.query(Resource).filter_by(emp_id=id).first()
        # resource = session.query(Resource).get(emp_id = id)
        resource = Resource.query.filter_by(emp_id=id).first()
        resources_manager  = resource.hiring_manager_id
        return redirect(url_for('resources_by_manager',manager_id = resources_manager ))
    tm_spocs = Employee.query.filter_by(position='TM SPOC').all()
    sl_pocs = Employee.query.filter_by(position='SL POC').all()
    return render_template('update_secondary_info.html', secondary_info=secondary_info, tm_spocs=tm_spocs, sl_pocs=sl_pocs)

@app.route('/delete_resource_by_manager/<int:id>')
# @jwt_required()
def delete_resource_by_manager(id):
    resource = Resource.query.get_or_404(id)
    resources_manager  = resource.hiring_manager_id
    secondary_info = ResourceSecondaryInfo.query.filter_by(id=resource.emp_id).first()
    if secondary_info:
